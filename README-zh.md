...
TUI 采用参考 Opencode 的深色主题和蓝色强调色，所有终端宽度统一使用顶部 Tab 导航。窄屏显示紧凑摘要，中屏和宽屏提供并排 Inspector；Overview 新增 transport session 健康、总带宽历史趋势和可操作告警，Topics 矩阵直接显示实时速率、带宽、丢弃数与稳定度。Logs 订阅本机 `/rosout`，保留最近 500 条记录。快捷键：`1`-`4` 切换页面，`f` 循环日志级别，`?` 打开帮助，`q` 后再次按 `q` 确认退出。直接调整终端大小即可重排，无需重启。最小验证尺寸为 `40x12`，日常使用建议至少 `80x24`。
